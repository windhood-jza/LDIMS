...
import { LoginRequest, ChangePasswordRequest, JwtPayload } from '../types/auth';
...
class AuthController {
  ...
  public async changePassword(req: Request, res: Response): Promise<void> {
    try {
      ...
      const successFlag = await AuthService.changePassword(userPayload.id, passwordData);
      ...
      if (error.message === '用户不存在' || error.message === '旧密码错误') {
         ...
         res.status(500).json(fail(error.message || '修改密码时发生服务器错误', 500));
      }
    }
  }
}
