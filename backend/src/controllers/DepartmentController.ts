...
import { CreateDepartmentRequest, UpdateDepartmentRequest } from "@ldims/types";
import { JwtPayload } from "@ldims/types"; // 引入 JwtPayload 类型
...
class DepartmentController {
  ...
  public createDepartmentValidation = [
    body("name")
      ...
      .isLength({ max: 50 })
      .withMessage("部门名称不能超过 50 个字符")
      .trim(), // Added trim
    body("code")
      .notEmpty()
      .withMessage("部门编码不能为空")
      .isLength({ max: 50 })
      .withMessage("部门编码不能超过 50 个字符")
      .matches(/^[a-zA-Z0-9_]+$/)
      .withMessage("部门编码只能包含字母、数字和下划线") // Added stricter pattern match
      .trim(), // Added trim
    body("parentId")
      .optional({ nullable: true })
      .isInt({ gt: 0 })
      .withMessage("父部门 ID 必须是正整数"),
    body("sortOrder").optional().isInt().withMessage("排序号必须是整数"),
    body("status")
      .optional()
      .isInt({ min: 0, max: 1 })
    ...
    body("parentId")
      ...
      .custom((value, { req }) => {
        ...
        if (
          req.params &&
          value !== null &&
          value === parseInt(req.params.id, 10)
        ) {
  ...
  public async updateDepartment(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (
        error.message.includes("已存在名为") ||
        error.message.includes("不能将部门")
      ) {
