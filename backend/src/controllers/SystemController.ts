...
export class SystemController {
    ...
    async updateConfigs(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            ...
            const configsToUpdate: SystemConfigMap = req.body;
             ...
             if (typeof configsToUpdate !== 'object' || configsToUpdate === null || Array.isArray(configsToUpdate)) {
     ...
     */
    async getLogs(req: Request<{}, {}, {}, OperationLogQuery>, res: Response, next: NextFunction): Promise<void> {
        try {
...
export const getLogsValidation = [
    ...
    query('pageSize').optional().isInt({ min: 1, max: 100 }).withMessage('页面大小必须在 1 到 100 之间'),
    query('userId').optional().isString().trim().escape(),
    query('operationType').optional().isString().trim().escape(),
    query('startDate').optional().isISO8601().toDate().withMessage('开始日期格式无效'),
    query('endDate').optional().isISO8601().toDate().withMessage('结束日期格式无效')
        .custom((value, { req }) => {
    ...
    query('sortField').optional().isIn(['createdAt', 'userId', 'operationType', 'username']).withMessage('无效的排序字段'),
    query('sortOrder').optional().isIn(['ASC', 'DESC']).withMessage('排序顺序必须是 ASC 或 DESC'),
    validateRequest
]; 