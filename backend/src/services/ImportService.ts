...
import { OperationType } from "@ldims/types"; // 添加导入
...
const EXCEL_COLUMN_MAP: Record<
  string,
  keyof DocumentImportDataBase | `ignore_${string}`
> = {
...
export class ImportService {
  ...
  async createImportTask(
    ...
    console.log(
      `[ImportService] Creating import task for user: ${userId}, original file: ${originalFileName}, uploaded file: ${uploadedFileName}`
    );
    ...
    const taskFileType = isValidExcelType
      ? (fileType as "xlsx" | "xls")
      : "unknown";
    if (!isValidExcelType) {
      // 如果文件类型无效，直接抛出错误，避免创建任务
      console.error(
        `[ImportService] Invalid file type detected: ${fileType} for file ${originalFileName}`
      );
      throw new Error(
        `无效的文件类型 '${fileType}'，请上传 Excel 文件 (.xls, .xlsx)`
      );
    }
    ...
    try {
      ...
      if (!fs.existsSync(filePath)) {
        console.error(
          `[ImportService] Uploaded file not found at path: ${filePath}.`
        );
      ...
      if (req) {
        ...
        console.warn(
          `[ImportService] Cannot log import task start for task ${newTask.id} because Request object is missing.`
        );
      }
      ...
      if (taskQueueService) {
        ...
        console.error(
          `[ImportService Critical Error] TaskQueueService not available when trying to add task ${newTask.id}. Dependency injection might have failed.`
        );
      ...
      if (error instanceof Error) {
        ...
        if ((error as any).original) {
          console.error(
            "[ImportService] Sequelize Original Error:",
        ...
        if (
          ...
          console.error(
            "[ImportService] Sequelize Validation Errors:",
            (error as any).errors
          );
        }
      } else {
      ...
      console.error(
        "[ImportService] Failed to create import task record. Raw error message:",
      ...
      const potentialFilePath = path.join(
        uploadsDir,
        path.basename(uploadedFileName)
      ); // 使用传入的 uploadedFileName
      if (fs.existsSync(potentialFilePath)) {
        try {
          fs.unlinkSync(potentialFilePath);
          console.log(
            `[API Cleanup] Deleted uploaded file due to task creation failure: ${potentialFilePath}`
          );
        } catch (unlinkErr) {
          console.error(
            `[API Cleanup] Failed to delete file ${potentialFilePath}:`,
            unlinkErr
          );
        }
      }
  ...
  async processImportTask(taskId: number): Promise<void> {
    ...
    try {
      ...
      if (!sheetName)
        throw new Error(`Excel no sheets: ${task.originalFileName}`);
      ...
      try {
        ...
        throw new Error(`Failed to parse sheet: ${e.message}`);
      }

      if (!jsonData || jsonData.length < 2)
        throw new Error("Excel empty or no header");

      const headerRow: string[] = jsonData[0].map((cell) =>
        String(cell).trim()
      );
      console.log(
        `[ImportService] Task ${taskId}: Headers: ${headerRow.join(", ")}`
      );
      ...
      const missingRequiredColumns = REQUIRED_EXCEL_COLUMNS.filter(
        (reqCol) => !headerRow.includes(reqCol)
      );
      if (missingRequiredColumns.length > 0) {
        throw new Error(
          `Excel is missing required columns: ${missingRequiredColumns.join(
            ", "
          )}`
        );
      }
      ...
      const requiredColumnIndices = REQUIRED_EXCEL_COLUMNS.map((reqCol) => {
        ...
        if (index === -1)
          throw new Error(
            `Logic error: Cannot find required column '${reqCol}' in header`
          );
      ...
      console.log(
        `[ImportService] Task ${taskId}: Processing rows (Total: ${totalRows})`
      );
      const columnMapIndices: Record<string, number> = {};
      headerRow.forEach((col, index) => {
        columnMapIndices[col] = index;
      });
      ...
      for (let i = 1; i < jsonData.length; i++) {
        ...
        for (const { colName, index } of requiredColumnIndices) {
          ...
          if (cellValue === null || String(cellValue).trim() === "") {
            errors.push({
              row: rowNumber,
              error: `Required column '${colName}' is empty`,
            });
            rowHasError = true;
            break;
          }
        ...
        if (rowHasError) {
          ...
          for (const excelColName in EXCEL_COLUMN_MAP) {
            const dbFieldName =
              EXCEL_COLUMN_MAP[excelColName as keyof typeof EXCEL_COLUMN_MAP];
            const colIndex = columnMapIndices[excelColName];
            if (typeof colIndex === "undefined") continue;
            const cellValue = rowData[colIndex];
            if (
              typeof dbFieldName === "string" &&
              dbFieldName.startsWith("ignore_")
            )
              continue;
            ...
            if (cellValue !== null && String(cellValue).trim() !== "") {
              ...
              if (dbFieldName === "handoverDate") {
                ...
                if (parsedDate) documentData.handoverDate = parsedDate;
                  ...
                  errors.push({
                    row: rowNumber,
                    error: `Invalid date format in column '${excelColName}': ${cellValue}`,
                  });
                  rowHasError = true;
                }
              } else if (dbFieldName === "docName")
                documentData.docName = trimmedValue;
              else if (dbFieldName === "docTypeName")
                documentData.docTypeName = trimmedValue;
              else if (dbFieldName === "sourceDepartmentName")
                documentData.sourceDepartmentName = trimmedValue;
              else if (dbFieldName === "submitter")
                documentData.submitter = trimmedValue;
              else if (dbFieldName === "receiver")
                documentData.receiver = trimmedValue;
              else if (dbFieldName === "signer")
                documentData.signer = trimmedValue;
              else if (dbFieldName === "storageLocation")
                documentData.storageLocation = trimmedValue;
              else if (dbFieldName === "remarks")
                documentData.remarks = trimmedValue;
            } else {
              if (!REQUIRED_EXCEL_COLUMNS.includes(excelColName)) {
                if (dbFieldName === "docTypeName")
                  documentData.docTypeName = null;
                else if (dbFieldName === "sourceDepartmentName")
                  documentData.sourceDepartmentName = null;
                else if (dbFieldName === "signer") documentData.signer = null;
                else if (dbFieldName === "storageLocation")
                  documentData.storageLocation = null;
                else if (dbFieldName === "remarks") documentData.remarks = null;
                else if (dbFieldName === "handoverDate")
                  documentData.handoverDate = null;
              } else {
                errors.push({
                  row: rowNumber,
                  error: `Required column '${excelColName}' is unexpectedly empty`,
                });
                rowHasError = true;
              }
            }
        ...
        task.progress =
          5 + Math.round(((task.processedRows || 0) / totalRows) * 85);

        if (i % 100 === 0 || i === jsonData.length - 1) {
          console.log(
            `[ImportService] Task ${taskId}: Processed ${task.processedRows}/${totalRows}. Progress: ${task.progress}%`
          );
          await task.save();
        }
      }
      ...
      console.log(
        `[ImportService] Task ${taskId}: Processed rows. Success: ${task.successCount}, Failure: ${task.failureCount}. Inserting...`
      );
      if (documentsToInsert.length > 0) {
        const transaction = await sequelize.transaction();
        try {
          await Document.bulkCreate(documentsToInsert as any[], {
            transaction,
            ignoreDuplicates: false,
          });
          await transaction.commit();
          console.log(
            `[ImportService] Task ${taskId}: Inserted ${documentsToInsert.length} documents.`
          );
          task.progress = 95;
        } catch (bulkError: any) {
          await transaction.rollback();
          console.error(
            `[ImportService] Task ${taskId}: Bulk insert failed:`,
            bulkError
          );
          task.failureCount =
            (task.failureCount || 0) + (task.successCount || 0);
          task.successCount = 0;
          errors.push({
            row: 0,
            error: `Bulk insert failed: ${bulkError.message}`,
          });
          throw new Error("Database error during bulk insert");
        }
      } else {
        console.log(
          `[ImportService] Task ${taskId}: No valid documents to insert.`
        );
        task.progress = 95;
      }
      ...
      if (errors.length > 0) {
        if (task.status === 3) {
          ...
          task.errorMessage =
            task.errorMessage ||
            `Import finished with ${errors.length} errors.`;
        }
        console.warn(
          `[ImportService] Task ${taskId} finished with errors. Status: ${task.status}. Errors: ${errors.length}`
        );
      } else {
        task.errorMessage = null;
        console.log(
          `[ImportService] Task ${taskId} completed successfully. Status: ${task.status}`
        );
      }
      await task.save();
      ...
      if (task?.status === 2) {
        // 成功
        logContent = `文档导入任务 #${taskId} (文件: ${
          task.originalFileName
        }) 完成，成功导入 ${task.successCount || 0} 条记录。`;
      } else if (task?.status === 3) {
        // 失败
        logContent = `文档导入任务 #${taskId} (文件: ${
          task.originalFileName
        }) 处理失败，成功 ${task.successCount || 0} 条，失败 ${
          task.failureCount || 0
        } 条。错误: ${task.errorMessage}`;
      } else {
      ...
      if (logUserId) {
        ...
        try {
          ...
          console.log(
            `[ImportService] Logged import task ${taskId} completion status.`
          );
        } catch (logError) {
          console.error(
            `[ImportService] Failed to log import task ${taskId} completion status:`,
            logError
          );
        }
      }
      ...
      if (task) {
        ...
        task.errorMessage =
          error.message || "Unknown error during import processing";
      ...
      console.log(
        `[ImportService] Finished task ${taskId}. Duration: ${duration.toFixed(
          2
  ...
  private parseExcelDate(cellValue: any): Date | null {
    ...
    if (typeof cellValue === "number") {
      try {
        ...
        console.warn(
          `[ImportService] Failed to parse Excel date number ${cellValue}:`,
          e
        );
        return null;
      }
