...
1.  **环境准备与项目初始化**:
    *   **后端 (Node.js + Express + TypeScript)**:
        *   [ ] 创建项目目录，初始化 `npm` (`npm init -y`)。
        *   [ ] 安装核心依赖：`express`, `mysql2`, `sequelize`, `@types/express`, `typescript`, `ts-node`, `nodemon` 等。
        *   [ ] 配置 `tsconfig.json` 进行 TypeScript 编译。
        *   [ ] 搭建基础目录结构：`src/` (包含 `routes`, `controllers`, `models`, `services`, `config`, `utils` 等)。
    *   **前端 (Vue3 + Vite + TypeScript + Element Plus)**:
        *   [ ] 使用 Vite 创建 Vue3 + TypeScript 项目 (`npm create vite@latest ldims-frontend --template vue-ts`)。
        *   [ ] 安装 `element-plus` 和 `vue-router`。
        *   [ ] 配置 Element Plus 按需引入或全局引入。
        *   [ ] 搭建基础目录结构：`src/` (包含 `components`, `views`, `router`, `services/api`, `styles`, `layouts` 等)。
    *   **数据库 (MySQL)**:
        *   [✅] 创建 `LDIMS_DB` 数据库 (已完成)。
    *   **版本控制**:
        *   [ ] 初始化 Git 仓库，进行首次提交。
...
2.  **数据库连接与模型**:
    *   **后端**:
        *   [ ] 在 `config` 中配置数据库连接信息（使用环境变量 `.env` 文件管理敏感信息如密码）。
        *   [ ] 实现 Sequelize 初始化和连接逻辑，**特别注意**：根据需求，即使连接失败，服务也应能启动，并在后续提供配置接口/提示。可以设计一个连接状态管理模块。
        *   [ ] 根据 `需求文档_LDIMS.md` 定义 Sequelize 模型 (`User`, `Department`, `DocType`, `Document`, `OperationLog`, `SearchCondition`, `ExportTask`)，建立模型间的关联关系 (Associations)。
...
4.  **基础服务与布局**:
    *   **后端**:
        *   [ ] 创建基础的 Express 应用实例，设置中间件 (如 `cors`, `body-parser`)。
        *   **前端**:
            *   创建基础布局组件 (`src/layouts/DefaultLayout.vue`)，包含侧边栏 (Sidebar)、顶部导航栏 (Header) 和主内容区域 (Main Content)，样式参考 Demo 文件。
            *   配置 Vue Router (`src/router/index.ts`)，设置基础路由结构，应用 `DefaultLayout`。

**阶段二：核心认证与基础管理模块 (骨架)**

5.  **认证模块 (Auth)**:
    *   **后端**: 实现用户登录接口 (`/auth/login`)，验证用户名密码，生成 JWT (JSON Web Token)。实现修改密码接口。实现 JWT 验证中间件，用于保护后续需要登录的接口。
    *   **前端**: 创建登录页面 (`src/views/Login.vue`)，样式参考 `login.html`。实现登录表单、API 调用和登录成功后的 Token 存储 (如 `localStorage`) 及页面跳转。设置路由守卫，未登录用户访问受保护页面时跳转到登录页。

6.  **用户管理模块 (User)**:
    *   **后端**: 实现用户管理的 CRUD (创建、读取、更新、删除) API 接口，包括列表查询（带分页、搜索、筛选）。
    *   **前端**: 创建用户管理页面 (`src/views/UserManagement.vue`)，包含用户列表 (Element Plus Table)、搜索栏、新增/编辑用户的弹窗 (Element Plus Dialog/Form)，样式参考 `user.html`。实现与后端 API 的对接。
