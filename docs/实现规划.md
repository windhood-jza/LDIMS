...
13. **系统设置与日志 (System)**:
    *   **后端**: 
        *   [✅] **模型 (`models/SystemConfig.ts`)**:
            *   [✅] 创建 `SystemConfig` 模型，包含字段：`id`, `configKey`, `configValue`, `description`。
    ...
    *   **前端**: 
        ...
        *   [✅] **API 调用层 (`services/api/system.ts`)**:
            *   [✅] 包含获取/更新配置、测试数据库连接和查询日志的方法。
        *   [ ] **权限控制**: 仅管理员可访问此设置页面 (包括日志查看)。

14. **仪表盘 (Dashboard)**:
    *   **后端**:
        *   [✅] **DTO/类型 (`types/dashboard.d.ts`)**:
            *   [✅] 定义 `DashboardSummaryData` 接口 (包含统计项、图表数据、最近日志、DB状态等)。
        *   [✅] **Service (`services/DashboardService.ts`)**:
            *   [✅] 创建 `DashboardService` 类。
            *   [✅] 实现 `getSummary()` 方法，聚合各 Service 数据。 (实际实现为 getSummaryData)
        *   [✅] **Controller (`controllers/DashboardController.ts`)**:
            *   [✅] 创建 `DashboardController` 类。
            *   [✅] 实现处理 `GET /api/v1/dashboard/summary` 请求的方法。
        *   [✅] **路由 (`routes/dashboard.ts`)**:
            *   [✅] 创建 `dashboard.ts` 路由文件。
            *   [✅] 定义 `GET /summary` 路由，应用认证。
        *   [✅] **主路由 (`routes/index.ts`)**: 
            *   [✅] 导入并挂载 `createDashboardRouter()`。
    *   **前端**:
        *   [ ] **视图 (`views/DashboardView.vue`)**: 
            *   [ ] 创建 `DashboardView.vue` 组件。
            *   [ ] **布局**: 使用 `el-row`, `el-col`, `el-card` 创建网格布局。
            *   [ ] **统计卡片区**: 显示文档总数、类型数、部门数等。
            *   [ ] **图表区**: 集成 ECharts，渲染文档类型和部门分布图。
            *   [ ] **快捷入口/系统状态区**: 添加常用操作链接，显示 DB 状态。
            *   [ ] **近期活动区**: 使用表格或列表显示最近操作日志。
        *   [ ] **API 调用层 (`services/api/dashboard.ts`)**:
            *   [ ] 创建 `dashboard.ts` 文件。
            *   [ ] 实现 `getDashboardSummary()` 函数。
        *   [ ] **状态管理**: 使用 `ref`/`reactive` 存储仪表盘数据和加载状态。
        *   [ ] **生命周期**: 在 `onMounted` 中调用 API 获取数据。
        *   [ ] **样式**: 应用适当样式。
