...
9.  **文档信息管理 (Document)**: (核心业务模块)
    ...
    *   **前端**:
        ...
        *   [✅] **搜索区域**:
            *   [✅] 添加 `<el-form>` 及表单项：关键字 (`<el-input>`)、文档类型 (`<el-tree-select>`)、所属部门 (`<el-tree-select>`)、签章人 (`<el-input>`)、交接日期范围 (`<el-date-picker type="daterange">`)。 *(字段名已根据后端模型调整)*
            *   [✅] 添加"搜索"、"重置"按钮 (`<el-button>`)。
            *   [✅] 实现 `handleSearch`, `resetSearch` 方法，更新查询参数并调用获取列表数据的方法 (`fetchData`)。
            *   [✅] 在 `onMounted` 中获取文档类型树和部门树数据，用于下拉选择 (`fetchDocTypes`, `fetchDepartments`)。
        *   [✅] **列表区域**:
            *   [✅] 添加 `<el-table>` 组件，绑定 `tableData` 和 `loading` 状态。
            *   [✅] 定义 `<el-table-column>` 显示文档信息：ID, 名称 (`docName`), 类型名称, 部门名称, 交接日期, 签章人, 创建人 (`createdBy`), 创建时间等。 *(字段名已根据后端模型调整，并添加了 submitter, receiver)*
            *   [✅] 添加操作列，包含"查看" (`openViewDialog` 占位)、"编辑" (`openEditDialog`)、"删除" (`handleDelete`) 按钮 (`<el-button link>`)。
            *   [✅] 实现日期、关联名称等的格式化显示 (`formatDate`, `formatDateTime`)。
        *   [✅] **分页组件**:
            *   [✅] 添加 `<el-pagination>` 组件，绑定分页状态 (`pagination.page`, `pagination.pageSize`, `pagination.total`)。
            *   [✅] 实现 `@size-change`, `@current-change` 事件处理函数 (`handleSizeChange`, `handleCurrentChange`)，更新分页参数并重新获取列表 (`fetchData`)。
        *   [✅] **新增/编辑弹窗 (`components/DocumentFormDialog.vue`)**:
            *   [✅] 创建独立的弹窗组件。
            *   [✅] 包含 `<el-dialog>` 和 `<el-form>`。
            *   [✅] 表单项对应文档字段：名称 (`docName`), 类型 (`el-tree-select`), 部门 (`el-tree-select`), 交接日期 (`el-date-picker`), 签章人 (`el-input`), 备注 (`el-input type="textarea"`) 等。 *(已根据后端模型调整，包含 submitter, receiver, storageLocation)*
            *   [✅] 通过 `props` 接收文档类型和部门数据用于下拉选择。
            *   [✅] 实现 `open(mode, data?)` 方法打开弹窗并初始化表单。
            *   [✅] 实现表单验证规则 (`rules`)。
            *   [✅] 实现 `handleSubmit` 方法，调用创建或更新 API (占位 `createDocument`, `updateDocument`)。
            *   [✅] 实现 `handleClose` 方法关闭弹窗。
            *   [✅] 提交成功后 `emit('success')` 事件通知父组件刷新列表。
        *   [✅] **API 调用层 (`services/api/document.ts`)**: (已填充具体请求逻辑，依赖 request 工具)
            *   [✅] 创建 `getDocuments`, `createDocument`, `getDocumentInfo`, `updateDocument`, `deleteDocument` 函数，调用后端 API。
        *   [ ] **状态管理**: 在 `DocumentListView.vue` 中使用 `ref` 和 `reactive` 管理搜索参数、表格数据、分页信息、加载状态、文档类型/部门列表等。 (已基本实现，后续可按需优化)
        *   [ ] **样式**: 参考 `document.html` demo 的视觉风格进行调整。 (已实现基础布局和组件，细节待微调)
