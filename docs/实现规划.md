        ...
        *   [✅] **操作日志**:
            ...
            *   [ ] **增强日志记录**: (分阶段进行)
                *   [ ] **阶段 1: 实现用户管理模块日志记录**
                    ...
                    *   [ ] **方案**: 采用 Sequelize Hooks (`afterCreate`, `afterUpdate`, `afterDestroy` for `User` model) 结合 AsyncLocalStorage 实现。
                    *   [ ] **上下文传递**: 配置中间件，使用 `AsyncLocalStorage` 在请求处理链中传递用户信息 (ID, Username)、请求信息 (IP, Path, Method)。
                    *   [ ] **模型集成 (User)**: 在 `User` 模型的 `init` 方法中添加 `hooks`，触发日志记录逻辑。
                    *   [ ] **日志服务**: 利用 `OperationLogService` 集中处理日志创建，从 `AsyncLocalStorage` 获取上下文信息。
                    *   [ ] **辅助函数 (可选)**: 考虑创建 `logModelActivity` 辅助函数简化 `User` Hooks 中的通用逻辑。
                    *   [ ] **日志内容 (User)**: 确保记录操作用户、操作类型、模块('用户管理')、对象ID/描述(用户名)、请求详情、操作状态及必要的细节（如字段变更，**注意：绝不记录密码原文**）。
                    *   [ ] **特殊操作 (User)**: 确保密码重置操作在 `UserService` 中被明确记录为 'PASSWORD_RESET' 类型。
                *   [ ] **阶段 2: 扩展到其他核心模块**
                    *   [ ] **目标**: 将日志记录扩展到部门管理、文档类型管理、文档管理模块。
                    *   [ ] **方案**: 复用阶段 1 的方案，在 `Department`, `DocType`, `Document` 模型中添加类似的 Sequelize Hooks。
                    *   [ ] **模型集成**: 在相应模型的 `init` 方法中添加 `hooks`。
                    *   [ ] **日志内容**: 调整 `logModelActivity` (如果使用) 或在 Hooks 中直接调用 `OperationLogService`，确保记录正确的模块名和对象描述。
    *   **前端**:
