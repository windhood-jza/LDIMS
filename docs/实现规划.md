...
11. **文档批量导入**:
    *   **后端**:
        ...
        *   [ ] **数据库迁移**:
            *   [ ] 提供 `ALTER TABLE export_tasks ADD COLUMN ...` SQL 语句添加新字段。
        *   [ ] **文件上传处理**:
            *   [ ] 安装 `multer` (`npm install multer @types/multer`)。
            *   [ ] 配置 `multer` (例如，设置临时存储目录 `uploads/`, 文件大小限制, 文件类型过滤 - 只允许 `.xlsx`, `.xls`)。
            *   [ ] 创建文件上传路由 (`POST /upload/excel`)，使用 `multer` 中间件处理上传，成功后返回文件的临时路径或唯一标识符。
        *   [ ] **服务层 (`services/ImportService.ts` 或扩展 `ExportService`)**:
            *   [ ] 创建 `createImportTask(userId: number, originalFileName: string, filePath: string): Promise<ExportTask>` 方法，用于在数据库创建类型为 `document_import` 的初始任务记录，并将任务 ID 添加到 `TaskQueueService`。
            *   [ ] 创建 `processImportTask(taskId: number): Promise<void>` 方法 (由 `TaskQueueService` 调用)。
                *   [ ] 更新任务状态为 `processing`。
                *   [ ] 安装并使用 `xlsx` 库 (`npm install xlsx`) 读取 Excel 文件。
                *   [ ] 逐行读取数据，记录 `totalRows`。
                *   [ ] **数据验证与处理**:
                    *   [ ] 定义 Excel 列与 `Document` 字段的映射关系。
                    *   [ ] 验证必填字段 (如文档名称)。
                    *   [ ] 根据空值处理规则处理空单元格。
                    *   [ ] (可选) 尝试根据部门/类型名称查找对应 ID (如果 Excel 提供的是名称)。
                    *   [ ] 调用 `DocumentService.create` 尝试插入数据。
                    *   [ ] 捕获创建错误，记录失败原因和行号到 `errorDetails`。
                    *   [ ] 更新 `processedRows`, `successCount`, `failureCount`。
                    *   [ ] 定期更新任务 `progress` (例如 `processedRows / totalRows * 100`)。
                *   [ ] 处理完成后，更新任务最终状态 (`completed` 或 `failed`) 和统计数据。
                *   [ ] (可选) 删除临时上传的文件。
        *   [ ] **路由/Controller (`routes/import.ts` 或扩展 `export.ts`)**:
            *   [ ] 创建 `POST /documents/import` 路由，接收上传成功后返回的文件路径/标识符，调用 `createImportTask`。
            *   [ ] (如果未复用) 创建 `GET /import-tasks`, `GET /import-tasks/:taskId` 路由获取导入任务列表和状态。
