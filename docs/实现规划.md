...
10. **文档查询与导出**:
    ...
    *   **前端**:
        *   [ ] **导出选项弹窗 (`components/ExportOptionsDialog.vue`)**:
            *   [ ] 创建组件，包含字段选择 (`<el-checkbox-group>`) 和文件类型选择 (`<el-radio-group>`)。
            *   [ ] 实现 `open` 方法和确认逻辑 (调用 `requestExport` API)。
        *   [ ] **列表页 (`DocumentListView.vue`)**: 添加"导出"按钮及打开弹窗逻辑。
        *   [ ] **导出任务列表页 (`views/ExportTaskList.vue`)**:
            *   [ ] 创建页面，使用 `<el-table>` 显示任务列表。
            *   [ ] 实现 API 调用、分页、下载、查看错误逻辑。
            *   [ ] (可选) 实现状态自动刷新。
        *   [ ] **API 调用层 (`services/api/export.ts`)**: 创建相关函数。
        *   [ ] **路由 (`router/index.ts`)**: 添加 `/export-tasks` 路由。
        *   [ ] **导航 (`layouts/DefaultLayout.vue`)**: 添加"导出任务"菜单项。
