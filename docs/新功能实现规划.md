...
- ✅ **文档创建/编辑流程调整 (`components/DocumentFormDialog.vue` 或类似组件):**
  ...
  - ✅ **文件管理区域 (条件渲染 `v-if="isMetadataSaved"` 已移除，改为始终显示):**
    - ✅ **显示当前文件列表 (`el-table`):** 列、数据、下载按钮。
    - ✅ **上传组件 (`el-upload`):** 配置、`v-model` 绑定、事件处理、提示。
    - ❌ `:before-upload` (可选)。(未明确实现)
...
- ✅ **内容展示 (卡片式画廊/操作卡片设计):**
  ...
  - ✅ **动态生成文件展示元素:** (原 Tabs 方案目标达成，方式变更为卡片)
    - ✅ 使用 `v-for` 遍历文档详情中的 `files` 数组生成卡片。
    - ✅ ~~每个 `<el-tab-pane>` 的 `label` 设为 `file.fileName`...~~ (不适用)
  - ✅ **每个文件卡片内容:**
    - ✅ 显示文件元数据：类型、大小、状态 (在卡片脚注或详情中)。
    - ✅ 添加文件的**下载/访问操作** (按钮或链接)。
    - ✅ **文件预览/访问区域 (根据 `file.fileType` 条件渲染):**
      - ✅ **图片 (jpg, png):** 使用卡片缩略图 + `<el-image-viewer>` 大图预览 (已优化)。
      - ✅ **PDF:** 提供 "在新标签页打开 PDF" 链接。
