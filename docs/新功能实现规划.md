...
- [ ] **系统设置页面 (`views/SettingsView.vue`):**
  ...
  - [ ] **获取配置:** 在页面加载时，调用后端 API (可能需要新增 GET /api/v1/system/config/:key 或 GET /api/v1/system/config) 获取 `FILE_STORAGE_PATH` 的当前值。
  - [ ] **UI 实现:** 添加一个表单项 (如 `<el-input>`)，标签为"文件存储路径"，用于显示和编辑该路径。
  - [ ] **保存逻辑:** 添加"保存"按钮，点击时调用 `PUT /api/v1/system/config` API，将 `configKey` 设为 `FILE_STORAGE_PATH`，`configValue` 设为用户输入的新路径。
  - [ ] **API 服务层 (`src/services/api/system.ts`):** (如果需要) 添加获取和更新系统配置的 API 调用函数。

- [ ] **文档创建/编辑流程调整 (`components/DocumentFormDialog.vue` 或类似组件):**

  - [ ] **状态管理:**
    - [ ] 添加响应式状态：`currentId` (文档 ID), `formData` (元数据), `files` (关联文件列表), `isMetadataSaved` (布尔值), `filesToUpload` (待上传 File 对象数组)。
  - [ ] **`open` 方法修改:**
    - [ ] 接收 `mode` ('add', 'edit', 'view') 和 `data` (可选，文档信息)。
    - [ ] 如果是 'add' 模式，重置所有状态，`isMetadataSaved = false`。
    - [ ] 如果是 'edit' 或 'view' 模式：
      - [ ] 使用传入的 `data` 或调用 `getDocumentById(id)` 获取完整信息 (包含 `files`)。
      - [ ] 更新 `formData` 和 `files` 状态。
      - [ ] 设置 `currentId`。
      - [ ] **设置 `isMetadataSaved = true`** (因为文档已存在)。
      - [ ] 重置 `filesToUpload`。
  - [ ] **元数据保存逻辑 (`handleSave` 或类似方法):**
    - [ ] 调用 `createDocument` 或 `updateDocumentMetadata` API (只发送元数据)。
    - [ ] **成功后:**
      - [ ] 显示成功消息。
      - [ ] **关闭对话框 (`dialogVisible = false`)。**
      - [ ] **触发 `success` 事件通知父组件刷新列表。**
      - [ ] (不再需要在此处获取文件列表或设置 `isMetadataSaved`)。
  - [ ] **文件管理区域 (条件渲染):**
    - [ ] 使用 `v-if="isMetadataSaved"` 或类似条件控制整个文件管理区域的显示。
    - [ ] **显示当前文件列表 (`el-table`):**
      - [ ] 数据源为 `files` 状态数组。
      - [ ] 列：序号、文件名、类型、大小、状态、下载按钮 (调用 `downloadFile(file.id)`)。
    - [ ] **上传组件 (`el-upload`):**
      - [ ] `action` 指向 `POST /api/v1/documents/${currentId}/files`。
      - [ ] 设置 `:headers` (认证 Token)。
      - [ ] `multiple=true`。
      - [ ] `:limit`。
      - [ ] **`:auto-upload="false"`。**
      - [ ] `:on-change` 保存文件到 `filesToUpload`。
      - [ ] `:on-exceed` 处理。
      - [ ] `:before-upload` (可选)。
      - [ ] `#tip` 显示替换提示。
    - [ ] **手动上传触发按钮 ("上传选中文件"):**
      - [ ] `v-if="filesToUpload.length > 0"`。
      - [ ] 点击时构造 `FormData`，调用 `uploadFiles(currentId, formData)` API。
      - [ ] 成功后：清空 `filesToUpload`，调用 `getDocumentById(currentId)` 刷新 `files` 列表。
    - [ ] **(可选) 清空按钮 ("清空所有文件"):**
      - [ ] `v-if="files.length > 0"`。
      - [ ] 确认后调用 `deleteAllFiles(currentId)` API。
      - [ ] 成功后刷新 `files` 列表 (应为空)。
    - [ ] **处理按钮 ("开始处理文件"):**
      - [ ] 根据 `files` 状态计算是否启用 (是否存在 'pending' 状态)。
      - [ ] 点击时调用 `startProcessing(currentId)` API。
      - [ ] 成功后可选择刷新 `files` 列表以更新状态，或显示提示。

- [ ] **内容展示 (Tabs - 可在 `DocumentFormDialog.vue` 的 `view` 模式下实现):**
...
- [ ] **文档列表页 (`views/DocumentListView.vue`):**
  - [ ] **修改表格列定义 (`<el-table-column>`):**
    ...
    - [ ] 保留核心元数据列。
    - [ ] **新增 "关联文件" 列:**
      - [ ] 显示文件数量 (`row.fileCount`) 和聚合状态。
      - [ ] **(新增) 添加 "文件管理" 按钮/链接:**
        - [ ] 显示图标或文字 "文件管理"。
        - [ ] 点击事件调用 `handleManageFiles(row)`。
      - [ ] **(修改) "编辑" 按钮:** 点击调用 `handleEdit(row)`。
      - [ ] **(修改) "查看" 按钮:** 点击调用 `handleView(row)`。
  - [ ] **添加处理函数:**
    - [ ] `handleManageFiles(row)`: 调用 `documentFormDialogRef.value.open('edit', row)` (或其他方式传递数据)。
    - [ ] `handleEdit(row)`: 调用 `documentFormDialogRef.value.open('edit', row)`。
    - [ ] `handleView(row)`: 调用 `documentFormDialogRef.value.open('view', row)`。
