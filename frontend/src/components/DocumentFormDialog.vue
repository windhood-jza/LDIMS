<template>
  ...
  >
    ...
    <!-- === 文件管理区域 === -->
    <div v-if="isMetadataSaved" class="file-management-area">
      ...
      <div style="margin-top: 20px">
        ...
        >
          <template #trigger>
            <el-button type="primary" :disabled="isViewMode"
              >选择文件</el-button
            >
          </template>
          <template #tip>
      ...
      <div style="margin-top: 15px; text-align: right">
        <el-button
          ...
          :disabled="isViewMode || associatedFiles.length === 0"
          :loading="loading"
          v-if="!isViewMode"
        >
          清空文件
        ...
        <el-button
          ...
          :disabled="isViewMode || filesToUpload.length === 0"
          :loading="loading"
          v-if="!isViewMode"
        >
          上传选中文件
...
const open = (type: "add" | "edit" | "view", data?: DocumentInfo) => {
  ...
  nextTick(() => {
    ...
    if ((type === "edit" || type === "view") && data) {
      ...
      console.log(
        "[Dialog] Data docTypeId:",
        data.docTypeId,
        typeof data.docTypeId
      );
      console.log(
        "[Dialog] Data sourceDepartmentId:",
        data.sourceDepartmentId,
        typeof data.sourceDepartmentId
      );

      currentId.value = data.id ?? null;
      associatedFiles.value = data.files || [];
      console.log(
        "[Dialog Open] Associated files (check fileName):",
      ...
      console.log(
        "[Dialog] FormData after population:",
        JSON.parse(JSON.stringify(formData))
      );
      console.log("[Dialog] Associated files:", associatedFiles.value);
      ...
      const typeNodeExists = props.docTypeTreeData?.some((node) =>
        checkNodeExists(node, docTypeIdForCheck)
      );
      const deptNodeExists = props.departmentTreeData?.some((node) =>
        checkNodeExists(node, deptIdForCheck)
      );
      console.log(
        `[Dialog] Node exists check in docTypeTreeData for ID ${data.docTypeId}?`,
        typeNodeExists
      );
      console.log(
        `[Dialog] Node exists check in departmentTreeData for ID ${data.sourceDepartmentId}?`,
        deptNodeExists
      );
    } else if (type === "add") {
...
const handleDownloadFile = async (
  file: DocumentFile & { downloading?: boolean }
) => {
