          ...
          <template #header>
            <div class="chart-header-container">
              ...
              <div class="chart-type-selector">
                <el-radio-group v-model="docTypeChartType" size="small" @change="handleDocTypeChartTypeChange">
        ...
        <el-card shadow="always" class="chart-card">
          <template #header>
            <div class="chart-header-container">
              ...
              <div class="chart-type-selector">
                <el-radio-group v-model="deptChartType" size="small" @change="handleDeptChartTypeChange">
                  <el-radio-button :value="'treemap'">矩形树图</el-radio-button>
                  <el-radio-button :value="'bar'">条形图</el-radio-button>
                  <el-radio-button :value="'pie'">饼图</el-radio-button>
                </el-radio-group>
...
import type { DashboardSummaryData } from '@backend-types/dashboard';
...
const generateColors = (count: number): string[] => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const shadeIndex = Math.min(Math.floor(i / colorGroups), baseColors[groupIndex].length - 1);
...
const handleDocTypeChartTypeChange = () => {
  if (summaryData.value && summaryData.value.charts && summaryData.value.charts.docsByType) {
...
const handleDeptChartTypeChange = () => {
  if (summaryData.value && summaryData.value.charts && summaryData.value.charts.docsByDepartment) {
...
const renderCharts = () => {
  ...
  nextTick(() => {
    ...
    setTimeout(() => {
      console.log('开始初始化图表...');
      try {
        if (summaryData.value?.charts.docsByType && summaryData.value.charts.docsByType.length > 0) {
          renderDocTypeChart(summaryData.value.charts.docsByType);
        } else {
          console.warn('文档类型图表数据为空');
        }
        
        if (summaryData.value?.charts.docsByDepartment && summaryData.value.charts.docsByDepartment.length > 0) {
          renderDepartmentChart(summaryData.value.charts.docsByDepartment);
        } else {
  ...
  try {
    ...
    switch (docTypeChartType.value) {
      ...
      case 'bar':
        ...
        const otherSum = sortedData.slice(15).reduce((sum, item) => sum + item.value, 0);
      ...
      case 'pie':
        ...
        const otherValue = sortedData.slice(10).reduce((sum, item) => sum + item.value, 0);
